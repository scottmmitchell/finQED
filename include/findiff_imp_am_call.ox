...
option_price_call_american_finite_diff_implicit(decl S, decl X,
						       decl  r, decl sigma, decl time, 
						       decl no_S_steps, decl no_t_steps) {
    decl sigma_sqr = sigma*sigma;

    decl M; if (imod(no_S_steps,2)==1) { M=no_S_steps+1; } else { M=no_S_steps; } // need no_S_steps to be even:
	...
	B = F;
	F = Ainv*B;
	for (decl m=1;m<M;++m) { 	// now check for exercise
